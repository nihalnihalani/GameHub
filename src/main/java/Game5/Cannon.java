...
public class Cannon {
	...
	private ArrayList<ConcreteBall> balls = new ArrayList<ConcreteBall>();
	...
	private Graphics2D drawBalls(Graphics2D g) {
		...
		if (GamePanel.click == true && GamePanel.cursorX > fireButtonX && GamePanel.cursorX < fireButtonX + fireButtonWidth && GamePanel.cursorY > fireButtonY && GamePanel.cursorY < fireButtonY + fireButtonHeight) {
			balls.add(new ConcreteBall(ballX, ballY, diameter, (int) (((double) power * -1) - ((double) power * -1) / ((double) 157) * (angle * -1)), (int) (((double) power * -1) / ((double) 157) * (angle * -1)), colorSelected));
